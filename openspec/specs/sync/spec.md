...
在面板中提供云端备份与恢复能力，支持连接阿里云盘账号、将面板配置与数据同步备份到云端、从云端恢复数据，并记录最近若干次备份历史供查看。

## Requirements

### Requirement: 阿里云盘连接管理

系统 SHALL 通过 aliyundrive-webdav 代理 + rclone webdav 后端连接阿里云盘。
...
系统 SHALL 将 data/notes/ 目录内容同步到阿里云盘。
...
系统 SHALL 在 /#/sync 提供云备份管理页面。

#### Scenario: 未连接状态
- **WHEN** 用户导航到 /sync
- **THEN** 页面显示阿里云盘连接引导：安装说明、WebDAV 配置表单

#### Scenario: 连接流程
- **WHEN** 用户点击「连接阿里云盘」
- **THEN** 页面引导安装 aliyundrive-webdav、扫码登录、填入 WebDAV 连接信息

#### Scenario: 已连接状态
- **WHEN** 用户已配置 WebDAV 连接
- **THEN** 页面显示连接状态、WebDAV 地址、最近同步时间
...
系统 SHALL 在配置页面检测 rclone 是否已安装，引导用户安装。
...
系统 SHALL 检测 aliyundrive-webdav 是否已安装。
